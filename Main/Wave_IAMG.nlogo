...
to recolor  ;; turtle procedure
  if not edge? and not driver?
    [ set size .5 set color scale-color yellow z -10 10 ]
end
...
to propagate   ;; turtle procedure -- propagates the wave from neighboring turtles
  set velocity (velocity + (stiffness * 0.01 * (sum [z] of neighbor-turtles - 4 * z)))
  set velocity (((100 - friction) / 100) * velocity)
end
